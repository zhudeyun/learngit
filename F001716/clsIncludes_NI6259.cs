...
namespace F001716
{
    public class clsIncludes_NI6259
    {
        #region Struct
        ...
        public clsIncludes_NI6259()
        {
            m_PortLine = new PortLine_t();

            //NiDioPort
            m_PortLine.Port = new Line_t[3][];
            m_PortLine.Port[0] = new Line_t[32];
            m_PortLine.Port[1] = new Line_t[8];
            m_PortLine.Port[2] = new Line_t[8];

            //NiAnaInLine
            m_PortLine.AnaInLine = new string[32];

            //NiAnaOutLine
            m_PortLine.AnaOutLine = new string[4];
        ...
        public void Init_Port_Line()
        {
            ...
            //NiDioPort0
            for (i_LineIndex = 0; i_LineIndex < 32; i_LineIndex++)
            {
                m_PortLine.Port[0][i_LineIndex].LineName = m_daqDevice + "/port0/line" + i_LineIndex.ToString();
                m_PortLine.Port[0][i_LineIndex].High = 1 << i_LineIndex;
                m_PortLine.Port[0][i_LineIndex].Low = 0;
            }

            //NiDioPort1/2
            for (i_PortIndex = 1; i_PortIndex < 3; i_PortIndex++)
            {
